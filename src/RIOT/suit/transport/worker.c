...
int suit_handle_url(const char *url)
{
    ...
    if (size < 0) {
        LOG_INFO("suit_worker: error getting manifest %d\n",size);
        return size;
    }
...
static void *_suit_worker_thread(void *arg)
{
    ...
    LOG_INFO("suit_worker: started.\n");
            ...
            gpio_write(GPIO_PIN(PORT_B, 7), 1);
    ...
    if (suit_handle_url(_url) == 0) {
        ...
        if (IS_USED(MODULE_SUIT_STORAGE_FLASHWRITE)) {
            ...
            gpio_write(GPIO_PIN(PORT_B, 7), 0);
            gpio_init(GPIO_PIN(PORT_B, 0), GPIO_OUT);
            gpio_write(GPIO_PIN(PORT_B,0), 1);
            xtimer_msleep(1000);
            gpio_write(GPIO_PIN(PORT_B,0), 0);
            pm_reboot();
        }
    }
    else {
