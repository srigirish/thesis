...
char* filePath = "status/prodTurntable.txt";
int sensor;
int num = 0;
...
char rx_handler_stack[2*THREAD_STACKSIZE_LARGE];
static msg_t _msg_q[1];

char module_motion_thread_stack[2*THREAD_STACKSIZE_LARGE];
#define COAP_INBUF_SIZE (256U)

/* Extend stacksize of nanocoap server thread */
static char _nanocoap_server_stack[3*THREAD_STACKSIZE_DEFAULT + THREAD_EXTRA_STACKSIZE_PRINTF];
#define NANOCOAP_SERVER_QUEUE_SIZE     (8)
static msg_t _nanocoap_server_msg_queue[2*NANOCOAP_SERVER_QUEUE_SIZE];

#define MAIN_QUEUE_SIZE     (8)
static msg_t _main_msg_queue[2*MAIN_QUEUE_SIZE];
...
int led_read(int* dev) {
	int res = 0;
	int temp[2];
	for(int i = 1; i<=2; i++) {
	temp[i-1] = adc_sample(ADC_LINE(i), RES);
	printf("LDR Value %d: %d\n",i,temp[i-1]);
	    if(temp[i-1] - value[i-1] > 45)
		*dev = i;
	value[i-1] = temp[i-1];	
	}
	res = adc_sample(ADC_LINE(*dev), RES);
	return res;
}

void led_write(unsigned int num, int dev) {
	int temp[8];
	gpio_t pin;
...
void *_module_motion_thread(void *arg)
{
   ...
   int ledValue = 0;
    ...
    sensor = gpio_read(GPIO_PIN(PORT_F, 13));
   
    if(state == 0) {
    while(num == 0)
    {
        ledValue = led_read(&num);
    }
    state = 1;
    printf("Sensor = %d, Num = %d\n",sensor,num);
    if(ledValue>0)
    xtimer_msleep(200);
    led_write(511, num);
    if(num == 1) {
