...
int led_read(void) {
	int temp = 0;
	uint64_t initSec = 0, finSec = 0;
	temp = adc_sample(ADC_LINE(1), RES);
	if(temp>55) {
	     initSec = xtimer_now_usec64();
	     while(adc_sample(ADC_LINE(1), RES)>55);
	     finSec = xtimer_now_usec64();
	}
	return (int)(finSec-initSec)/1000;
	//return temp;
}
...
void led_write(unsigned int num) {
	...
	for(int i = 0; i<8; i++) {
	if(temp[i]==1)
	gpio_write(GPIO_PIN(PORT_E, 8), HIGH);
	else
	gpio_write(GPIO_PIN(PORT_E, 8), LOW);
	xtimer_msleep(100);
	}
	printf("\n");
	gpio_write(GPIO_PIN(PORT_E, 8), LOW);
...
char* filePath = "status/slider.txt";
int sensor;
...
//    int ledValue = 0;
  ...
  sensor =  gpio_read(GPIO_PIN(PORT_B, 10));
  //printf("State: %d\n",state);
  if (state == 0) {
 rxMessage = 'f';

while(!(strcmp(rx_buf,"ProdTTA") == 0 || strcmp(rx_buf,"ProdTTB") == 0))
{
...
if(strcmp(rx_buf, "ProdTTA") == 0)
rxMessage = 'a';
else if(strcmp(rx_buf, "ProdTTB") == 0)
rxMessage = 'b';
  ...
  state = 3;
    ...
    while(!(strcmp(rx_buf,"DisTTF") == 0 || strcmp(rx_buf,"DisBF") == 0)) {
...
void moveSlider(char direction) {
  if (direction == 'b') {
  while(gpio_read(GPIO_PIN(PORT_E, 0)) != 0) {
     gpio_write(GPIO_PIN(PORT_D, 15), HIGH);
     gpio_write(GPIO_PIN(PORT_F, 12), LOW);
  }
  }
  else if (direction == 'f') {
  while(gpio_read(GPIO_PIN(PORT_B, 11)) != 0) {
...
int main(void)
{
   ...
   gpio_init(GPIO_PIN(PORT_E, 7), GPIO_OUT);	//4
   gpio_init(GPIO_PIN(PORT_D, 13), GPIO_OUT);	//5
   gpio_init(GPIO_PIN(PORT_A, 0), GPIO_OUT);	//6	//high away from storage
   gpio_init(GPIO_PIN(PORT_E, 15), GPIO_OUT); 	//9	
   gpio_init(GPIO_PIN(PORT_F, 12), GPIO_OUT);	//10    //high towards conveyor A
   gpio_init(GPIO_PIN(PORT_D, 15), GPIO_OUT);	//11
   gpio_init(GPIO_PIN(PORT_E, 8), GPIO_OUT);	//LiFi LED
   
   gpio_init(GPIO_PIN(PORT_B, 10), GPIO_IN_PD);	//2	//sensor	
   gpio_init(GPIO_PIN(PORT_B, 11), GPIO_IN_PU);	//30	//near conveyor A
   gpio_init(GPIO_PIN(PORT_E, 0), GPIO_IN_PU);	//32
   if(adc_init(ADC_LINE(1))<0)
   printf("Error initialising ADC!!!!\n");
   
   gpio_write(GPIO_PIN(PORT_E, 7), HIGH);
   gpio_write(GPIO_PIN(PORT_D, 13), HIGH);
   moveSlider('b');
