...
PLiFi lifi1(GPIO_PIN(PORT_E, 7), GPIO_PIN(PORT_A, 0), on_rx, NULL); //22,0
...
static void control_message(const uint8_t *msg, uint8_t len)
{
    if (len > 0)
    {
        ...
        if (control_data[0] == sink2[1])
        {
            // Serial.println("TurnTable ready");
            state = 2;
            state2 = 4;
        }
        if (control_data[0] == sink2[2])
        {
            state = 3;
            state2 = 0;
            userRequest = '0';
...
static void request_message(const uint8_t *msg, uint8_t len)
{
    if (len > 0)
    {
        ...
        if (control_data[0] == pallet_types[6])
        {
            userRequest = '0';
            gpio_write(GPIO_PIN(PORT_D, 13), LOW);  //Red
...
device_id=0x03;
    ...
    gpio_write(GPIO_PIN(PORT_B, 3), HIGH);
...
while(1)
{
    ...
    startSensor = gpio_read(GPIO_PIN(PORT_A, 9));
    endSensor = gpio_read(GPIO_PIN(PORT_A, 8));

    // if(endSensor!=digitalRead(5)) {
    //     messageSent = false;
    // }

    if (startSensor && (userRequest == '0' || userRequest == 'b'))
    {
        state2 = 1;
    }
